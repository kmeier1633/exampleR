...
calc_rates <- function(){
  ...
  # bring in data
  brffed <- import_clean_data(type = "brokered")
...
import_clean_data <- function(type = "brokered"){
  
# get data in the format we want - all rates & vols for each FFED rate
# aka append the FF,ED data for each (already combined for FR2420)
...
# brokered daily volumes by rate
...
# restricted by the min of the max dates available...(it is updated daily for the ED file)
  ...
  ### fed funds csv file is not updating regularly
...
###################################################
  ...
  if(type == "fr2420"){
   ...
   source("rfunction.R")
   ## ## ## ## ##
   fr2420 <- getFR2420(ff.ed.filters=TRUE,overnight.only=TRUE)
   # restrictions
   fr2420 <- fr2420[fr2420$trade_type %in% c("FF","ED"),]
   fr2420 <- fr2420[!is.na(fr2420$rate) & !is.na(fr2420$maturity_date),]
   fr2420 <- fr2420[fr2420$overnight == TRUE,]
   # keep what was used before
   ffed <- as.data.frame(fr2420)[c("dt","id_rssd","rate","amount",
